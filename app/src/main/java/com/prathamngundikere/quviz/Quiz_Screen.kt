...
class Quiz_Screen : AppCompatActivity() {
    ...
    private var sec: Int = 60*1000
    private var min: Int = 4
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_quiz_screen)
        timeview = findViewById(R.id.tv_timer)
        startTimer(sec.toLong())
        quizName = intent.getStringExtra("quizname")
        val questionlists = questions(quizName)
        recyclerView = findViewById(R.id.quizscreenrv)
        val selectedOptions = SparseArray<Int>()
        quizquestionAdapter = QuizQuestionAdapter(questionlists,selectedOptions)
        recyclerView.adapter = quizquestionAdapter
        val submit_btn = findViewById<Button>(R.id.submitbutton)
    ...
    private fun extracted(quizName: String?) {
        countDownTimer?.cancel()
        val intent = Intent(this, ResultScreen::class.java)
        intent.putExtra("quizname", quizName)
        intent.putExtra("timetaken", timeview.text.toString())
    ...
    private fun startTimer(durationinMillis: Long)
    {
        ...
        countDownTimer = object : CountDownTimer(durationinMillis,1000) {
            override fun onTick(millisUntilFinished: Long) {
                ...
                {
                    timeview.text = "0$min:0$secondsRemaining"
                }else {
                    timeview.text = "0$min:$secondsRemaining"
                }
            }
    ...
    private fun questions(quizName: String?): List<question> {
